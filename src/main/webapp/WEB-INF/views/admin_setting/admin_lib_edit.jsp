...
<body>
		...
		<div id="content-wrapper">
			<div class="container-fluid">
				...
				<form class="horizontal" name="edit_admin_lib" method="post" action="${pageContext.request.contextPath}/admin_setting/admin_lib_edit_ok.do">
					...
					<div class="form-group mt-2">
						<div class="col-md-offset-2 col-md-6">
							...
							<div class="float-right">
								<a href="#" class="btn btn-info btn-sm" data-toggle="modal" data-target="#delete_book_modal">도서삭제</a>
					...
					<div class="form-inline mb-2">
						<c:set var="yesAutoBrwRtn" value=""/>
						<c:set var="noAutoBrwRtn" value=""/>
						<c:choose>
							<c:when test="${item.kioskMode eq 1}">
								<c:set var="yesAutoBrwRtn" value="checked"/>
							</c:when>
							<c:otherwise>
								<c:set var="noAutoBrwRtn" value="checked"/>
							</c:otherwise>
						</c:choose>
						<label for="" class="col-3">무인대출반납</label>
						<span class="col-9">
							<span class="col-6">
								<label for="autoUse">사용</label>
								<input type="radio" id="autoUse" name="kioskMode" value="1" ${yesAutoBrwRtn}/>
							</span>
							<span class="col-6">
								<label for="autoNotUse">미사용</label>
								<input type="radio" id="autoNotUse" name="kioskMode" value="0" ${noAutoBrwRtn}/>
							</span>
						</span>
					</div>
				...
				<div>
					...
					<c:url var="viewUrl" value="/blook_around.do">
						<c:param name="lib" value="${item.idLib}" />
						<c:param name="skl" value="${item.stringKeyLib}" />
					</c:url>
					<a href="${viewUrl}" onclick="window.open(this.href, '_blank','width=550,height=600,scrollbars=yes');return false;">
						${pageContext.request.contextPath}/blook_around.do?lib=${item.idLib}&skl=${item.stringKeyLib}
					</a>
				</div>
			...
			<div class="modal-content">
				...
				<form name="form_admin_lib_delete" method="post">
					...
					<div class="modal-body">
						...
						<div class="text-danger">모든 정보가 삭제되어 돌이킬수 없습니다. 반드시 확인해주세요.</div>
					</div>
					<div class="modal-footer">
						<button class="btn btn-secondary" type="button"
							data-dismiss="modal">취소</button>
	...
	<div class="modal fade" id="delete_book_modal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">도서를 삭제하시겠습니까?</h5>
					<button class="close" type="button" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">×</span>
					</button>
				</div>
				<form name="form_admin_book_delete" method="post">
					<input type="hidden" name="idLib" value="${item.idLib}"/>
					<div class="modal-body">
						<div>등록된 도서가 모두 삭제 됩니다.</div>
						<div class="text-danger">모든 도서가 삭제되어 돌이킬수 없습니다. 반드시 확인해주세요.</div>
					</div>
					<div class="modal-footer">
						<button class="btn btn-secondary" type="button"
							data-dismiss="modal">취소</button>
						<input type="submit" class="btn btn-danger" value="삭제" formaction="${pageContext.request.contextPath}/admin_setting/admin_book_delete_ok.do"/>
					</div>
				</form>
			</div>
		</div>
