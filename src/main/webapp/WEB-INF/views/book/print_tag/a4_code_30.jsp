...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/assets/css/book/print_label/a4_code_30.css" />
</head>
<body>

	<div class="prtNone">
		<span class="prtNone" style="font-size:30pt;">청구기호 30칸</span>
		<span class="prtNone">
	...
	<div class="whole__box">
		...
		<c:forEach var="item" items="${bookHeldList}" varStatus="status">
				...
				<div class="data__box">
					...
					<c:choose>
						<c:when test="${fn:length(item.localIdBarcode) gt 10}">
							<div class="barcode" style="font-size:0.9em;">${item.localIdBarcode}</div>
						</c:when>
						<c:otherwise>
			...
			<c:if test="${(((status.count+startPoint-1) mod 30) eq 0) and ((fn:length(bookHeldList)+startPoint-1) gt 30)}">
