...
public class WebHelper {
	/** 쿠키에서 사용할 도메인 */
	//private static final String DOMAIN = "finebook.co.kr";
	private static final String[] DOMAINS2 = {"localhost", "finebook.co.kr"};
	...
	public void init() {
		...
		// --> 24시간
		this.session.setMaxInactiveInterval(60 * 60 * 12);
	...
	// msg 위치 매개변수에 숫자가 들어가면, redirect로 아래 audio 태그로 사운드 재생
	// --> import org.springframework.web.servlet.ModelAndView;
	public ModelAndView redirect(String url, int SoundEff) {
		// 가상의 View로 만들기 위한 HTML 태그 구성
		String html = "<!doctype html>";
		html += "<html>";
		html += "<head>";
		...
		// 메시지 표시
		if (SoundEff != 0) {
			html += "<audio autoplay src=\"/files/upload/finebook4/common_sound/reg_ok_sound.mp3\"></audio>";
		}
		html += "</body>";
		html += "</html>";
	...
	public String[][] getString2DArray(String fieldName, String[][] defaultValue) {
		// 리턴을 위한 값을 두 번째 파라미터(기본값)로 설정해 둔다.
		String[][] result = defaultValue;
		// 배열 형태의 GET,POST 파라미터를 받는다.
		String[][] param = (String[][]) this.request.getAttribute(fieldName);
		if (param != null) { // 수신된 파라미터가 존재한다면?
			if (param.length > 0) { // 배열의 길이가 0보다 크다면?
				result = param; // 리턴을 위해서 준비한 변수에 수신한 값을 복사한다.
			}
	...
	public String[][] getString2DArray(String fieldName) {
		return this.getString2DArray(fieldName, null);
	}
	...
	public void setCookie(String key, String value, int timeout) {
		...
		for(int i=0; i<DOMAINS2.length; i++) {
			if(DOMAINS2[i].indexOf(request.getServerName())> -1) {
				thisDomain = DOMAINS2[i];
				break;
			}
		}
